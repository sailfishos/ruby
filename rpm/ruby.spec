...
%global gem_extdir_mri %{gem_archdir}/%{name}/%{gem_name}-%{version}%{?prerelease}
...
Source15: generate_bootstrap.sh
...
Provides: ruby(runtime_executable) = %{ruby_version}
...
License:    Ruby or BSD
Provides:   ruby(release) = %{ruby_version}
...
%prep
%autosetup -p1 -n %{name}-%{ruby_version}/%{name}
...
cp -a %{SOURCE3} .
# Push our bootstrap sources that we generated before
# in to the sources
%if %{with_bootstrap}
cp -r ../bootstrap/. .
%endif
...
# http://bugs.ruby-lang.org/issues/7807
sed -i 's/Version: \${ruby_version}/Version: %{Version}/' %{buildroot}%{_libdir}/pkgconfig/%{name}.pc

# Kill bundled certificates, as they should be part of ca-certificates.
rm -rf %{buildroot}%{rubygems_dir}/rubygems/ssl_certs/*
